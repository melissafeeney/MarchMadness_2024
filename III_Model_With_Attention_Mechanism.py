...
# -------------------------
# III. GENERATE MODEL WITH ATTENTION MECHANISM
# -------------------------
...
def fit_model(X_train, y_train):
  # Input layer for combined difference features
  input_layer = Input(shape = (num_features,))

  dense_layer1 = Dense(32, activation = hidden_layer_act)(input_layer)
  dense_layer2 = Dense(16, activation = hidden_layer_act)(dense_layer1)  

  attention_layer = Attention(use_scale = True, dropout = 0.2, seed = 123)([dense_layer2, dense_layer2])

  concatenated_features = Concatenate()(
      [dense_layer2, attention_layer])

  x = Dense(8, activation = hidden_layer_act)(concatenated_features)
  outputs = Dense(units = 1, activation = 'sigmoid')(x)  # Output for binary win prediction

  # Create model
  model = Model(inputs = input_layer, outputs = outputs)

  # Compile model (adjust loss and metrics based on your needs)
...
def define_stacked_model(members):
  ...
  # add new layers
  hidden1 = Dense(64, activation = hidden_layer_act)(merged)
  hidden2 = Dense(32, activation = hidden_layer_act)(hidden1)
  hidden3 = Dense(16, activation = hidden_layer_act)(hidden2)
  attention_layer = Attention(use_scale = True, dropout = 0.2, seed = 123)([hidden3, hidden3])

  # Concatenate weighted sum and original features
  concatenated_features = Concatenate()(
      [hidden3, attention_layer])

  # DNN layers for classification
  x = Dense(8, activation = hidden_layer_act)(concatenated_features)
  output = Dense(units = 1, activation = 'sigmoid')(x)  # Output for binary win prediction
